    ...
    <nav className={darkMode?"navDark":null}>
        <h1><a href="/#home">Tyler Wertman Developments</a></h1>
            <div id={darkMode ? "navLinksL" : "navLinksL"}>
              ...
              <Link className={hoveredElement === 4 ? 'shadowNav' : 'hover'} onMouseEnter={()=>{hover(4)}} onMouseLeave={hover} to={"/#contact"}><img src={hoveredElement === 4 ? contactLG : !darkMode ? contactDSG : contactLG} alt="contact"/><span className={hoveredElement === 4 ? 'navLiHover' : darkMode ? 'test' : 'hidden'}>&nbsp;Contact</span></Link>&nbsp;
            ...
            <div id="navLinksSE" className={isNavOpen ? "navLinksSE" : "navLinksSE hide"} onClick={()=>{setIsNavOpen(false)}}>
              {/* <img src="" alt="" /> */}
              <Link className={hoveredElement === 0 ? 'shadowNav' : 'hover'} onMouseEnter={()=>{hover(0)}} onMouseLeave={hover} to={"/#home"}><img src={hoveredElement === 0 ? homeDSG : homeLG} alt="home"/><span className={hoveredElement === 0 ? 'navLiHover' : 'hidden'}></span></Link>&nbsp;
              <Link className={hoveredElement === 1 ? 'shadowNav' : 'hover'} onMouseEnter={()=>{hover(1)}} onMouseLeave={hover} to={"/#about"}><img src={hoveredElement === 1 ? aboutDSG : aboutLG} alt="about"/><span className={hoveredElement === 1 ? 'navLiHover' : 'hidden'}></span></Link>&nbsp;
              <Link className={hoveredElement === 2 ? 'shadowNav' : 'hover'} onMouseEnter={()=>{hover(2)}} onMouseLeave={hover} to={"/#projects"}><img src={hoveredElement === 2 ? projectsDSG : projectsLG} alt="projects"/><span className={hoveredElement === 2 ? 'navLiHover' : 'hidden'}></span></Link>&nbsp;
              <Link className={hoveredElement === 3 ? 'shadowNav' : 'hover'} onMouseEnter={()=>{hover(3)}} onMouseLeave={hover} to={"/#education"}><img src={hoveredElement === 3 ? educationDSG : educationLG} alt="education"/><span className={hoveredElement === 3 ? 'navLiHover' : 'hidden'}></span></Link>&nbsp;
              <Link className={hoveredElement === 4 ? 'shadowNav' : 'hover'} onMouseEnter={()=>{hover(4)}} onMouseLeave={hover} to={"/#contact"}><img src={hoveredElement === 4 ? contactDSG : contactLG} alt="contact"/><span className={hoveredElement === 4 ? 'navLiHover' : 'hidden'}></span></Link>&nbsp;
            </div>
    </nav>
  )
}
